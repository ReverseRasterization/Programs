...
void draw_rect(sf::VertexArray& m_verticies, sf::Vector2i size, sf::Vector2f position, float round_factor, float round_resolution) {
    ...
    m_verticies.resize(18 + ((num_triangles*3) * 4));
    ...
    sf::Vertex nVertex;
    nVertex.color = sf::Color::White; // Set the color of all vertices
    ...
    //Bottom Left Corner
    lastPeak = {position.x, (position.y + size.y)-loss}; // this serves as the outer-point

    for (int i=0;i<num_triangles; i++) {
        int j = (18 + ((num_triangles * 3)*2) + (i*3));

        nVertex.position = {position.x + loss, (position.y + size.y)-loss}; // origin
        //nVertex.color = sf::Color::Red;
        m_verticies[j] = nVertex;

        nVertex.position = lastPeak;
        //nVertex.color = sf::Color::Blue;
        m_verticies[j + 1] = nVertex;
        ...
        sf::Vector2f peak = {(position.x+loss) - (static_cast<float>(std::cos(currRad))*loss), ((position.y + size.y)-loss) + (static_cast<float>(std::sin(currRad)) * loss)};
    ...
    //Top Left Corner
    lastPeak = {position.x, position.y+loss}; // this serves as the outer-point

    for (int i=0;i<num_triangles; i++) {
        int j = (18 + ((num_triangles * 3)*3) + (i*3));

        nVertex.position = {position.x + loss, position.y + loss}; // origin
        //nVertex.color = sf::Color::Red;
        m_verticies[j] = nVertex;
        ...
        sf::Vector2f peak = {(position.x+loss) - (static_cast<float>(std::cos(currRad))*loss), (position.y+loss) - (static_cast<float>(std::sin(currRad)) * loss)};
